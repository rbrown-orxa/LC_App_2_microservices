...
def control(P_sun, P_load, E_sto, dt, E_rated):
    ...
    """
    P_nl = P_load - P_sun 
    # Net load -> Power needed by building 
    #from grid or battery (or combination)
    # outputs:
    ...
    # Energy stored in the battery at the end of current timestep (kWh)
    
    if P_nl>0: 
    # (load > sun) -> Try to discharge the battery to the building load, 
    # if energy is available
        #TODO: Limit rate of discharge of battery
        E_next = E_next if E_next>0. else 0.
        P_sto = (E_next - E_sto)/dt 
        # <0 # Calculate power flowing out of the battery
        P_grid = P_nl + P_sto 
        # Calculate power taken from the grid as load power 
        # minus the batter discharge power
    else: # (load <= sun) -> Try to charge the battery
        E_next = E_next if E_next<E_rated else E_rated 
        # Calculate battery charge state. Don't charge if full.
        P_sto = (E_next - E_sto)/dt # >0 
        # Calculate power flowing into the battery
        P_curt = -P_nl - P_sto
        #Calculate exported power: difference between 
        # net load and power flowing into battery
...
def check_result(E_sto, E_rated, digits=3):
    ...
    try:
        assert round( E_sto, digits ) >= 0.0, \
            'Battery charge state is negative'
        assert round ( E_sto, digits ) <= round( E_rated, digits) , \
            'Battery charge state is greater than capacity'
...
def run_control(results, timestep, size, generation, _load, _dt):
    ...
    (results['P_sto'][timestep],
        results['P_grid'][timestep],
        results['P_curt'][timestep]) = \
        control(
            generation.values[timestep],
            _load.values[timestep],
            results['E_sto'][timestep],
            dt=_dt,
            E_rated=size)
    
    results['E_sto'][timestep+1] = (
                                    results['E_sto'][timestep] 
                                    + results['P_sto'][timestep] * _dt)
...
def stop_or_continue(cost, prev_cost, keep_iterating):
    ...
    keep_iterating = ( (keep_iterating+1) 
                        if  diff_cost <= 0 
                        else (keep_iterating-1) )
...
def simulate_battery_perfomance(load, generation, batt_cost, import_cost, 
                                timestep_hrs, export_price):
    rv = []
    size, prev_cost, keep_iterating = 0, 0, 1
    
    logging.info('Evaluating battery optimiser results')
    ...
    while keep_iterating:
        ...
        results, costs = evaluate_results(
            results, 
            batt_cost, 
            size, 
            import_cost, 
        ...
        prev_cost, keep_iterating = stop_or_continue(
            costs['total_cost'],
            prev_cost, keep_iterating )

        size += 1

    print('\n', end='')
    logging.info('Finished optimising battery')
...
def get_timestep_in_hours(load_profiles_df):
    try:
        ...
        raise TypeError(
            'Unable to determine timestep from load profile data')
    return timestep
...
def optimise_battery_size(schema,df,pv_size):
    ...
    dict = get_fixed_fields(schema,
        fields=['battery_cost_kwh',
                'battery_life_cycles',
                'import_cost_kwh',
                'export_price_kwh'])
