...
def get_ev_loads(schema):
    ...
    list_of_ev_loads=[]
    dict = get_variable_fields(schema, fields=['num_ev_chargers','building_type'])
    building_type = dict['building_type']  
    no_of_ev_chargers = dict['num_ev_chargers']   
    for btype,no_ev_charger in zip(building_type,no_of_ev_chargers):
        list_of_ev_loads.append(_process_ev_load(building_type=btype,charge_points=no_ev_charger ))
        
    return(list_of_ev_loads)


def _process_ev_load(building_type='domestic',ratio=0.2,start_date=None,charge_points=1):
    """
    ...
    if start_date is not None:
        ...
        building_EV_load_one_year = df * charge_points        
        #rename the column
        building_EV_load_one_year.rename(columns={building_type:'kWh'},inplace=True)
